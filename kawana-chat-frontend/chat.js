...
//
// Backend contract preserved — same SSE event shape from server.js. This
// frontend sends only { session_id, message }; the backend resolves WHICH
// deployment (demo-kawana) from the request Origin, so there is no slug here.
...
(() => {
  ...
  // Static greeting + clickable example questions + a call/email CTA to book a
  // free measure & quote. Kawana has no online booking form — the route in is
  // phone/email, matching the deployment's routing_close. Renders on boot and
  // on "New chat". Cleared the moment the customer sends their first message.
  const PHONE = '(07) 5493 9540';
  const PHONE_TEL = '0754939540';
  const EMAIL = 'info@kfloor.com.au';
  const EXAMPLE_QUESTIONS = [
    'What’s the difference between hybrid, vinyl and laminate?',
  ...
  function renderWelcome() {
    ...
    wrap.innerHTML =
      '<h2 class="welcome-title">Hello — welcome to Kawana Flooring.</h2>' +
      '<p class="welcome-body">I’m here to answer quick questions about ' +
      'flooring options, how our process works, and where we service.</p>' +
      '<p class="welcome-cta">Ready for a quote? Call ' +
      '<a href="tel:' + PHONE_TEL + '">' + PHONE + '</a> or email ' +
      '<a href="mailto:' + EMAIL + '">' + EMAIL + '</a> to book a free measure ' +
      '&amp; quote — no pressure, no obligation.</p>' +
  ...
  function linkify(el) {
    ...
    // consumes each address whole, so info@kfloor.com.au is linked as one
    // mailto and its domain is never re-matched as a URL. The email
    ...
    let html = escaped.replace(linkPattern, (match) => {
      ...
      // (e.g. "...page," or "(page)."), re-appending it after the anchor.
      let trail = '';
      const m = match.match(/[.,;:!?)\]]+$/);
